...
export class AutoScheduleGenerator {
   ...
   */
  private getCustomersForDay(date: Date, _dayOfWeek: DayOfWeek): Customer[] {
    return this.customers.filter(customer => customerNeedsServiceOnDate(customer, date));
  }
  ...
  private getAvailableCrewsForDay(date: Date, dayOfWeek: DayOfWeek): Map<string, User[]> {
    ...
    this.crews.forEach((crewMembers, crewId) => {
      const availableMembers = crewMembers.filter(member => {
        return isUserAvailableOnDay(member, dayOfWeek) && member.status !== 'offline';
      });
  ...
  private assignCustomersToCrew(
    ...
    const crewServiceTypes = crewMembers[0]?.crewServiceTypes || [];

    const compatibleCustomers = customers.filter(customer =>
      crewCanServiceCustomer(crewServiceTypes, customer)
    );
